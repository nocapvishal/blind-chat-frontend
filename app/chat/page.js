...
export default function ChatPage() {
  ...
  const sendMessage = () => {
    ...
    // WhatsApp-like keyboard persistence
    setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f7] dark:bg-[#0f0f11] pb-32">
      ...
      {showIsland && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 bg-black text-white px-6 py-2 rounded-full shadow-2xl backdrop-blur-xl animate-spring z-50">
          New message
        </div>
      )}
  ...
  return (
    <div className={`relative max-w-[75%] ${msg.self ? "ml-auto" : ""}`}>
      ...
      >
        ...
        {msg.reaction && (
          <div className="absolute -bottom-3 right-3 text-sm animate-spring">
            {msg.reaction}
          </div>
        )}
      </div>
      ...
      {showReactions && (
        <div className="absolute -top-12 bg-white dark:bg-[#1c1c1e] shadow-lg rounded-full px-3 py-2 flex gap-2 animate-spring">
          {["❤️","😂","🔥","👍","😮"].map((emoji) => (
            <span
              key={emoji}
              onClick={() => {
